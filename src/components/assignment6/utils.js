...
function groupByCity(data) {
    ...
    let result = data.reduce((result, d) => {
        // Increment the count for the destination city
        let destCity = result[d.DestCity] || {
            "City": d.DestCity,
            "Count": 0
        };
        destCity.Count += 1;
        result[d.DestCity] = destCity;

        // If the source city is different from the destination, increment its count too
        if (d.DestCity !== d.SourceCity) {
            let sourceCity = result[d.SourceCity] || {
                "City": d.SourceCity,
                "Count": 0
            };
            sourceCity.Count += 1;
            result[d.SourceCity] = sourceCity;
        }

        return result;
    }, {});

    // Transform the results object into an array of city objects
    return Object.keys(result).map(key => result[key]).sort((a, b) => b.Count - a.Count);
}
